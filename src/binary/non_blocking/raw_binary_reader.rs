...
use crate::result::{decoding_error, decoding_error_raw, illegal_operation, illegal_operation_raw, incomplete_data_error};
use crate::{IonResult, IonType, RawStreamItem, RawSymbolToken};
use bytes::Buf;
use std::io::Read;
use std::ops::Range;
...
impl<A: AsRef<[u8]>> RawBinaryBufferReader<A> {
    ...
    pub fn step_in(&mut self) -> IonResult<()> {
        let value = self.encoded_value()
            .ok_or_else(|| illegal_operation_raw(format!("cannot step in; the reader is not positioned over a container")))?;
        ...
        let container_type = match value.header.ion_type.unwrap() {
            ...
            other => return illegal_operation(format!("cannot step into a {}; it is not a container", other))
        };
...
impl<'a, A: AsRef<[u8]>> TxReader<'a, A> {
    ...
    fn is_at_end_of_container(&self) -> bool {
        if let Some(parent) = self.parent {
            if self.main_buffer.total_consumed() + self.tx_buffer.total_consumed() >= parent.exclusive_end {
                return true;
            }
        }
        return false;
...
mod tests {
    ...
    fn expect_annotations<A: AsRef<[u8]>, I: IntoAnnotations>(reader: &RawBinaryBufferReader<A>, annotations: I) {
